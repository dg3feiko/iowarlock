...
module.exports = function(redis){
  ...
  if(redis_client.defineCommand)
  {
	  redis_client.defineCommand('parityDel', {
		  numberOfKeys: 1,
		  lua: require('fs').readFileSync(__dirname + '/lua/parityDel.lua').toString()
	  })
  }else
  {
	  scripty = new Scripty(redis_client);
  }
  ...
  warlock.unlock = function(key, id, cb) {
	...
	if(redis_client.defineCommand)
	{
		redis_client.parityDel(warlock.makeKey(key), id, cb);
	}else
	{
